...
function Item({id, rules, setRules}: {id: number, rules:Rule[], setRules: React.Dispatch<React.SetStateAction<Rule[]>>}) {
    ...
    const handleEdit = () => {
        setRules(rules.map(r => r.id === id? {...r, name: name, keyword: keyword}: r))
                ...
                <Dialog>
                    ...
                    <DialogContent>
                        <div className="flex flex-col gap-4">
                            ...
                            <Label className="mt-4" htmlFor="keyword">Keyword</Label>
                            <Input id="keyword" value={keyword} onChange={handleKeyword}></Input>
                        </div>
                        <DialogClose onClick={handleEdit} className="h-[6vh] bg-primary rounded-md text-black">
                            FINISH
                        </DialogClose>
                    </DialogContent>
                </Dialog>
...
function AddItem({rules, setRules}: {rules:Rule[], setRules: React.Dispatch<React.SetStateAction<Rule[]>>}) {
    ...
    const handleAdd = () => {
        const sortRule = rules.map((r, i) => {return {...r, id: i}})
        setRules([...sortRule, {id: sortRule.length, name: name, keyword: keyword}])
        setName("")
        setKeyword("")
    }
        ...
        <div className="w-[12vw] h-[6vh] border hover:bg-accent rounded-md p-2">
            <Dialog>
                ...
                <DialogContent>
                    ...
                    <DialogClose onClick={handleAdd} className="h-[6vh] bg-primary rounded-md text-black">
                        ADD
                    </DialogClose>
                </DialogContent>
            </Dialog>
